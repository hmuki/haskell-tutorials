...
splitEachFive l
    | length l > 5  = (take 5 l) : splitEachFive (drop 5 l)
    | otherwise     = [l ++ (take (5-(length l)) ['X','X'..])]
prop_transpose :: String -> Bool
prop_transpose l = splitEachFive l == transpose (transpose (splitEachFive l))
...
unite xss
    | xss == [] = "" 
    | otherwise   = (head xss) ++ (unite (tail xss))
encrypt :: Int -> String -> String
encrypt k str  = unite (transpose (splitEachFive (encipherStr k str)))
   
-- 16.

decrypt :: Int -> String -> String
decrypt k str = undefined
